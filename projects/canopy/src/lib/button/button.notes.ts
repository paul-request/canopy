...
The \`ButtonIconPosition\` property can be used to locate the icon on the left or right hand side of the text.
Content projection is used as Canopy Icons need loading via the iconRegistry which enables tree shaking of icons.

~~~html
<button lg-button type="button" [ButtonIconPosition]="left" >
  Add item
  <lg-icon name="add" />
</button>
~~~
...
The \`iconButton\` property is used to visually hide that text from sighted users.
...
| \`\`iconPosition\`\` | The position of the icon in the button | string | left | No |
...
| \`\`lg-btn--icon-right\`\` | If the button contains a element with class .lg-icon it will align the element to the right  |
| \`\`lg-btn--icon-left\`\` | If the button contains a element with class .lg-icon it will align the element to the left  |
